...
class EnvConfig:
    """ 数据中心场景参数设置 —— twitter_xlargescale (20 节点 × 20 微服务)

    由 config_twitter_largescale (10n×10ms) 全等比放大 2× 得到，用于回应二次意见 ① / R1-1
    ...
    全等比缩放（node : ms : flow = 20 : 20 : 20，与 large 的 10:10:10 严格一致）：
      ...
      - C                 : 35 → 70  (≈ 3.5 × node_nums)
      - estimated_max_lamda : 50 (仅观测归一化用，per-MS 负载与 large 一致故不翻倍)
      - 其余字段（trace、init_lamda、奖励权重、训练超参）与 largescale 保持一致。

    设计沿革：
      - v1 取 ms=15（仅 ×1.5）→ flows/MS=2.0、per-MS 到达率翻倍(lamda 15→31)、RSR 暴跌。
      - v2 取 node=ms=flow=30（×3）→ per-MS 负载正确，但 30 MS 方差大 + 训练成本高。
      - v3（当前）取 node=ms=flow=20（×2）：per-MS 负载与 large 完全可比，训练可承受，
        动作空间 20×20×7=2800 (vs large 700，4×)，规模卖点充分。
    """
    ...
    def __init__(self):
        ...
        self.max_instance_update_num = 3    # 测试：回调到与 large 一致
        ...
        self.estimated_max_lamda = 50   # 仅用于观测归一化；per-MS 负载现与 large 一致，归一化上限同步回归
        self.init_lamda = 15
