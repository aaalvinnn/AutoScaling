...
class EnvConfig:
    """ 数据中心场景参数设置 —— sin_xlargescale (20 节点 × 20 微服务)

    由 config_sin_largescale (10n×10ms) 全等比放大 2× 得到，与 config_twitter_xlargescale
    ...
    全等比缩放（node : ms : flow = 20 : 20 : 20，与 large 的 10:10:10 严格一致）：
      ...
      - C                 : 35 → 70  (≈ 3.5 × node_nums，与 twitter/alibaba xlarge 同比例)
      - min/max_request_T : 10/20 → 20/40  (large 的 2 倍，大规模放宽时延约束)
      - estimated_max_lamda : 50 (仅观测归一化用，per-MS 负载与 large 一致故不翻倍)
      - device            : cpu → cuda:0  (大规模需 GPU 训练，与 twitter_xlargescale 一致；large 用 cpu 仅因规模小)
      - 其余字段（trace loads-sin、init_lamda=15、奖励权重、训练超参）与 largescale 保持一致。
    """

    def __init__(self):
        # id
        self.config_name = "sin_xlargescale"
        # 随机数种子
        self.seed = 1037
        ...
        # 训练配置
        self.device = "cuda:0"   # 大规模需 GPU 训练，与 twitter_xlargescale 一致 (large 用 cpu 仅因规模小)
