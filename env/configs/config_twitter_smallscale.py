...
class EnvConfig:
    ...
    def __init__(self):
        # id
        self.config_name = "twitter_smallscale"
        # 随机数种子
        self.seed = 1037
        ...
        # 用户、请求链配置
        self.request_flow_nums = 4
        self.min_request_chain_length = 3
        self.max_request_chain_length = min(5, self.ms_nums)
        self.estimated_max_lamda = 40
        self.init_lamda = 15
        ...
        self.C = 12    # 服务提供商给出的时间平均长期开销预算
        self.Q_max = 7
        self.Q_min = 0

        # 模型、算法配置
        self.predicter_window_size = 20
        self.history_lamda_length = min(10, self.node_nums*2)   # 记录的历史到达率，长度不能大于(self.node_nums*self.ms_nums)，否则无法储存于tensor中；用于模型训练的状态输入
