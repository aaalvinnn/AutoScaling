    ...
    with tqdm(total=CONFIG.num_iterations, desc="Training", unit="it") as pbar:
        for iteration in range(1, CONFIG.num_iterations + 1):
            ...
            for step in range(0, CONFIG.num_steps):
                ...
                try:
                    # 传入step后是按列截取的，而我们期望是按行截取的，故转置一下。同时提供多进程保护
                    action_np = action.detach().cpu().numpy().copy().T  
            ...
            if len(record_reward) < 100:
                pbar.set_postfix(reward=np.sum(record_reward))
            else:
                pbar.set_postfix(reward=np.sum(record_reward[-100:]))
            pbar.update(1)
...
if __name__ == "__main__":
    ...
    agent.load("model/twitter_largescale/0424/2036_V100_0/PPO_dnn")
    train(agent)