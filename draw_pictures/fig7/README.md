...
- `data/bw_{BW}_{t_all,cost,rsr}.npy` — 各 BW 设置下的逐时隙指标，**由 `draw.py` 自包含生成**：脚本扫描 T={1,2,3,4,5}（BW=D/T），每个设置用已训练 LGDRL 模型跑一个 episode rollout 落盘 npy，再画归一化折线图。
- **模型**：`model/twitter_largescale/0530/1829/PPO_dnn/model_dnn_best.pth`（观测不含带宽，策略决策不变，无需重训）。
- **复现**：直接 `python draw.py`（会重新 rollout 覆盖 `data/bw_*.npy` 并出图）。
